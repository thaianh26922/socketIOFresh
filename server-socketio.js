...
var io = require("socket.io")(server, {
   cors: {
      origin: ["http://localhost:4200", "http://localhost", "ionic://localhost", "http://127.0.0.1:3000"],
      methods: ["GET", "POST"]
   }
});
var listRoom = {};
...
io.on("connection", (socket) => {
      ...
      try {
         ...
         if (!Object.keys(listRoom).some(room => room === roomName)) {
            listRoom[roomName] = {
               ...
               // ...data,
               socketIDUserCreate: socket.id,
               member: [{ username: data.userId, role: "master", socketID: socket.id, roomName }],
            };
            socket.broadcast.emit("listRoom", Object.values(listRoom));
            // console.log(`  ~ socket.id`, socket.id)
         }
   ...
   socket.on("validateLoginRoom", ({ room, userID }) => {
      ...
      try {
         ...
         socket.emit('errorSocket', { msg: error.message, at: 'validate Login Room' })
   ...
   socket.on("resultJoinRoom", (data) => {
      try {
         ...
         socket.emit('errorSocket', { msg: error.message, at: 'result foin room' })
         console.log(`  ~ error`, error)
      }
   });
   ...
   socket.on("joinRoom", ({ room, userID }) => {
      ...
      //end not used
      let role = 'user';
      const checkMaster = listRoom[room]?.member
         .filter((item) => item.username === userID && item.role === 'master');
      if (checkMaster && checkMaster.length > 0) role = 'master';

      if (!drawData[room]) drawData[room] = [];

      if (!layer[room]) layer[room] = 1;
      if (!layerStorage[room]) layerStorage[room] = [
         {
            id: 1,
            canvas: {
      ...
      socket.on("fetch-data-request", (room) => {
         ...
         socket.emit("fetch-data-to-client", { drawData: drawData[room], layer: layer[room], layerStorage: layerStorage[room], role: role });
      });
      ...
      socket.on("disconnect", function (data) {
         if (listRoom[room]) {
            const { socketIDUserCreate } = listRoom[room];
            if (socketIDUserCreate === socket.id) {
